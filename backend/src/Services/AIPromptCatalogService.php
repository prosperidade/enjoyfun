        ...
        return [
            'marketing' => [
                ...
                'system_prompt' => implode("\n\n", [
                    ...
                    '- get_ticket_demand_signals(event_id) — sinais de demanda de ingressos: velocidade de vendas, lotes, conversao.',
                    '- get_ticket_sales_snapshot(event_id) — snapshot de vendas de ingressos: vendidos, disponiveis, receita por lote.',
                    '- get_event_kpi_dashboard(event_id) — KPIs consolidados do evento: receita, headcount, ocupacao.',
            ...
            'logistics' => [
                ...
                'system_prompt' => implode("\n\n", [
                    ...
                    '## FERRAMENTAS DISPONIVEIS',
                    '- get_parking_live_snapshot(event_id) — ocupacao, fluxo de entrada/saida, pendencias de bip.',
                    '- get_event_shift_coverage(event_id) — cobertura de workforce por setor e turno.',
                    '- get_shift_gaps(event_id) — lacunas de cobertura de turnos que precisam de atencao.',
                    '- get_meal_service_status(event_id) — status de servico de refeicoes: planejado vs servido por janela.',
                    '- find_events(name_query?) — SOMENTE se o evento nao estiver selecionado.',
                    '## RESTRICOES',
            ...
            'contracting' => [
                ...
                'system_prompt' => implode("\n\n", [
                    ...
                    '## FERRAMENTAS DISPONIVEIS',
                    '- get_artist_schedule(event_id) — agenda de artistas: bookings, horarios de show, status de confirmacao.',
                    '- get_artist_logistics_status(event_id) — status logistico: voos, hoteis, transfers, itens pendentes por artista.',
                    '- get_finance_summary(event_id) — resumo financeiro: receita vs custo, margem, contas a pagar.',
                    '- find_events(name_query?) — SOMENTE se o evento nao estiver selecionado.',
                    '## RESTRICOES',
            ...
            'data_analyst' => [
                ...
                'system_prompt' => implode("\n\n", [
                    ...
                    '- get_event_kpi_dashboard(event_id) — KPIs consolidados do evento: receita, headcount, ocupacao.',
                    '- get_finance_summary(event_id) — resumo financeiro: receita vs custo, margem, contas a pagar.',
                    '- get_pos_sales_snapshot(event_id, time_filter?) — vendas de PDV: faturamento, top produtos, ticket medio. Suporta time_filter.',
                    '- get_ticket_demand_signals(event_id) — sinais de demanda de ingressos: velocidade, conversao, lotes.',
                    '- find_events(name_query?) — SOMENTE se o evento nao estiver selecionado.',
                    '## FILTRO TEMPORAL',
                    'Quando o usuario perguntar "vendas de hoje", "do dia", "agora", OBRIGATORIAMENTE passe time_filter="24h" na tool get_pos_sales_snapshot. O campo "period" no resultado confirma o recorte temporal — mencione-o na resposta.',
                    '## RESTRICOES',
