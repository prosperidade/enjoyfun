...
export default function Sidebar({ isOpen, onClose }) {
  ...
  useEffect(() => {
    ...
    api.get("/organizer-settings")
      .then(({ data }) => {
        ...
        setBrand((prev) => ({ ...prev, ...payload }));
        applyBrand(payload);
        localStorage.setItem("enjoyfun_brand", JSON.stringify({ ...brand, ...payload }));
      })
      .catch(() => {});
  ...
  return (
    <>
      ...
      <aside
        className={`
        fixed top-0 left-0 h-full w-64 sidebar-bg sidebar-border flex flex-col z-50
      ...
      >
        <div className="h-16 px-6 border-b sidebar-border flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            {brand.logo_url ? (
              <img src={brand.logo_url} alt="Logo" className="w-8 h-8 rounded-lg object-cover border border-white/10" />
            ) : (
              <div className="w-8 h-8 rounded-lg bg-brand-gradient flex items-center justify-center brand-glow">
                <Zap size={16} className="text-white" />
              </div>
            )}
            <span className="font-bold text-white text-lg tracking-tight truncate">{brand.app_name || "EnjoyFun"}</span>
          </div>
        ...
        <nav className="flex-1 px-3 py-6 overflow-y-auto space-y-1 scrollbar-hide">
          <div className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3 px-3 text-[10px]">Menu Principal</div>
                    ...
                    <div className="pl-9 space-y-1">
                      {item.subItems.map((sub) => (
                        <NavLink
                          ...
                          onClick={onClose}
                          className={({ isActive }) =>
                            `flex items-center gap-3 px-3 py-2 rounded-lg text-xs font-medium transition-all ${isActive ? "text-brand bg-brand-soft" : "text-gray-500 hover:text-gray-300"}`
                          }
                        >
                          <span>{sub.label}</span>
                        </NavLink>
          ...
          <div className="mt-8">
            <div className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3 px-3 text-[10px]">Sistema</div>
            <NavLink
              to="/settings"
              className={({ isActive }) =>
        ...
        <div className="p-6 border-t sidebar-border">
          <p className="text-[10px] text-center text-gray-600 font-medium uppercase tracking-widest">EnjoyFun v2.0</p>
        </div>
      </aside>
    </>
  );
